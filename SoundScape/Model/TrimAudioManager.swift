...
protocol TrimAudioManagerDelegate: AnyObject {
    func didExport(to url: URL)
}

class TrimAudioManager {
    
    static let shared = TrimAudioManager()
    
    private var asset: AVURLAsset?
    
    private let outputFileName = "output"
    
    weak var delegate: TrimAudioManagerDelegate?
    ...
    func trimAudio(from head: Double, to tail: Double) {
        ...
        print("message from TrimAudioManager: oriigin audio length: \(length) seconds")
